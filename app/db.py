...
from .user_context import current_storage_key
...
class Database:
    ...
    def active_path(self) -> Path:
        """Resolve the current user's physically isolated learning database.

        Only the canonical learning.db is tenant-routed. Temporary/test databases and
        the global account database are never redirected.
        """
        storage_key = current_storage_key()
        if storage_key and self.path.name == "learning.db":
            base_dir = Path(os.getenv("LLM_LEARNING_DATA_DIR", str(self.path.parent)))
            return base_dir / "users" / storage_key / "learning.db"
        return self.path

    @property
    def backup_dir(self) -> Path:
        return self.active_path.parent / "backups"
    ...
    def list_thoughts(self, lesson_key: str | None = None) -> list[dict[str, Any]]:
        ...
        if lesson_key:
            sql, params = sql + " WHERE lesson_key=?", (lesson_key,)
    ...
    def get_thought(self, item_id: int) -> dict[str, Any] | None:
        with self.connect() as conn:
            row = conn.execute("SELECT * FROM thoughts WHERE id=?", (item_id,)).fetchone()
    ...
    def save_thought(self, item_id: int | None, *, title: str, content: str, tags: str, lesson_key: str | None, language: str) -> int:
        ...
        with self.connect() as conn:
            if item_id:
                conn.execute(
                    "UPDATE thoughts SET title=?,content=?,tags=?,lesson_key=?,language=?,updated_at=? WHERE id=?",
                    (title, content, tags, lesson_key or None, language, ts, item_id),
                )
                return item_id
            cur = conn.execute(
                "INSERT INTO thoughts(title,content,tags,lesson_key,language,created_at,updated_at) VALUES(?,?,?,?,?,?,?)",
                (title, content, tags, lesson_key or None, language, ts, ts),
            )
    ...
    def save_resource(self, item_id: int | None, *, title: str, url: str, description: str, tags: str, lesson_key: str | None, language: str) -> int:
        ...
        with self.connect() as conn:
            if item_id:
                conn.execute(
                    "UPDATE resources SET title=?,url=?,description=?,tags=?,lesson_key=?,language=?,updated_at=? WHERE id=?",
                    (title, url, description, tags, lesson_key or None, language, ts, item_id),
                )
                return item_id
            cur = conn.execute(
                "INSERT INTO resources(title,url,description,tags,lesson_key,language,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)",
                (title, url, description, tags, lesson_key or None, language, ts, ts),
            )
            return int(cur.lastrowid)
