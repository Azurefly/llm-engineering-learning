...
def course(request: Request, lesson_key: str):
    ...
    context = nav_context(request,lang); context.update({"lesson":lesson,"lesson_title":title_for(lesson,lang),"content_html":render_markdown(text),"source":source,"lesson_progress":progress,"lesson_thoughts":db.list_thoughts(lesson_key)[:6],"lesson_resources":db.list_resources(lesson_key)[:6]})
    return templates.TemplateResponse(request=request, name="course.html", context=context)

@app.post("/course/{lesson_key}/progress")
def update_progress(lesson_key: str, status: str=Form(...), percent: int=Form(0), score: str=Form("")):
    if lesson_key not in LESSON_MAP: raise HTTPException(404)
    if status not in {"not_started","in_progress","completed"}: status="not_started"
    try: parsed_score = float(score) if score.strip() else None
    except ValueError: parsed_score = None
    db.set_progress(lesson_key,status,percent,parsed_score); return RedirectResponse(f"/course/{lesson_key}",status_code=303)
